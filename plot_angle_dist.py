...
fspl_ls = real_data['fspl_ls'].transpose(1,0)
...
for iang in range(AngleFormat.nangle):
    ...
    for j in range(2):
        ...
        fspl_ls = data['fspl_ls'].transpose(1,0)
        
        # nlos_pl_ls = np.zeros((cfg.nfreq, data['nlos_pl_ls'].shape[1], data['nlos_pl_ls'].shape[1]))
        nlos_pl_ls = data['nlos_pl_ls'].transpose(1,0,2)

...
with open('dict_Tpath_whole.p', 'rb') as handle:
...
for ilink in range(nlink):
    fspl_ls = data['fspl_ls'][ilink,:]
    kpath = None
    for ipath in range(npaths_max):
        max_pl_consider_in_lowest_freq = max_ex_pl + fspl_ls[0]
        if data['nlos_pl_ls'][ilink, 0, ipath] == max_pl_consider_in_lowest_freq:
            kpath = ipath
            break
    if kpath is not None:
        # data['npath'][ilink] = kpath
        # data['npath_matched'][ilink] = kpath
        for ifreq in range(1, len(fspl_ls)):
            data['nlos_pl_ls'][ilink, ifreq, kpath:] = fspl_ls[ifreq] + max_ex_pl
            data['nlos_ang'][ilink, kpath:, :] = 0.0
            data['nlos_dly'][ilink, kpath:] = 0.0

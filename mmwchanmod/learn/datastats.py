...
def combine_nlos_los(data, cfg):
    ...
    # pl_ls = np.zeros((cfg.nfreq, data['nlos_pl'].shape[0],data['nlos_pl'].shape[1])) # (nfreq, nlink, npath)
    # data['nlos_pl_ls'] => (nlink, nfreq, npath)
    
    pl_ls = data['nlos_pl_ls'].transpose(1,0,2)
    ...
    for ifreq in range(cfg.nfreq):
        ...
        pl_ls[ifreq,Ilos,0] = data['los_pl_ls'][Ilos,ifreq]

...
class ChanMod(object):
    ...
    def __init__(self, cfg=None, nlatent=50,\
        ...
        self.path_discriminator_weights_fn = 'path_disc_weights.h5'
    ...
    def fit_link_mod(self, train_data, test_data, epochs=50, lr=1e-4):
        ...
        self.link_hist = self.link_mod.fit(\
                Xtr,ytr, batch_size=1024, epochs=epochs, validation_data=(Xts,yts) )            
    ...
    def inv_transform_pl(self, fspl_ls, Xpl):
        ...
        for i in range(self.nfreq):
         ...
         # if self.nfreq == 2:
        if self.nfreq > 1:
            for ifreq in range(1, self.nfreq):
                pl_omni = -10*np.log10(np.sum(10**(-0.1*nlos_pl_ls[0,:,:]),axis=1))
                pl2_omni = -10*np.log10(np.sum(10**(-0.1*nlos_pl_ls[ifreq,:,:]),axis=1))
    
                idx = np.where(pl2_omni <= (pl_omni + 20*np.log10(self.freq_set[ifreq]/self.freq_set[0])))[0]
    
                for i in idx:
                    valid_path_idx = np.where(nlos_pl_ls[0,i,:] != np.max(nlos_pl_ls[0,i,:]))[0]
                    nlos_pl_ls[ifreq,i,valid_path_idx] = nlos_pl_ls[ifreq,i,valid_path_idx] + 20*np.log10(self.freq_set[ifreq]/self.freq_set[0])
    ...
    def sample_path(self, dvec, fspl_ls, rx_type, link_state=None, return_dict=False):
        ...
        # data['nlos_pl_ls'] = nlos_pl_ls_record[ifreq,:,:]
        # data['los_pl_ls'] = los_pl_ls_record[ifreq,:]
        # data['fspl_ls'] = fspl_ls[ifreq,:]
        data['nlos_pl_ls'] = nlos_pl_ls_record.transpose(1,0,2)
        data['los_pl_ls'] = los_pl_ls_record.transpose(1,0)
        data['fspl_ls'] = fspl_ls.transpose(1,0)
    ...
    def save_path_model(self):
        ...
        disc_weights_path = os.path.join(self.model_dir, self.path_discriminator_weights_fn)
            
        # Save the GAN weights
        self.path_mod.generator.save_weights(weigths_path, save_format='h5')
        self.path_mod.discriminator.save_weights(disc_weights_path, save_format='h5')
    ...
    def fit_path_mod(self, train_data, test_data, epochs=50, lr=1e-4,\
                     checkpoint_period = 500, save_mod=True, batch_size=512,\
                     d_steps=5, gp_weight=10,  pre_trained_generator_path=None,\
        ...
        pre_trained_generator_path: str, optional
            File path to the pre-trained generator model.
        pre_trained_discriminator_path: str, optional
            File path to the pre-trained discriminator model.
        """      
        # Get the link state
        gen_loss = []
        dsc_loss = []
        ...
        for ifreq in range(self.nfreq):
            ...
            train_fspl_ls[ifreq,:] = train_data['fspl_ls'][Itr][:,ifreq]
            train_nlos_pl_ls[ifreq,:,:] = train_data['nlos_pl_ls'][Itr,:self.npaths_max][:,ifreq]
            # if ifreq == 0:
        ...
        if pre_trained_generator_path and os.path.exists(pre_trained_generator_path):
            generator.load_weights(pre_trained_generator_path)
            print("Loaded pre-trained generator weights.")

        if pre_trained_discriminator_path and os.path.exists(pre_trained_discriminator_path):
            discriminator.load_weights(pre_trained_discriminator_path)
        ...
        for epoch in range(epochs):
            ...
            if epoch % checkpoint_period == 0:
                ...
                self.path_mod.discriminator.save(weigths_path + f'/discriminator-epochs-{epoch}.h5')

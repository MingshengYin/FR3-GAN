...
class MPChan(object):
    ...
    def rms_angle(self, fspl_ls):
        ...
        no_path = 0
        if self.link_state == LinkState.no_link:
            rms_aoa_phi_ls = [0] * nfreq
            rms_aoa_theta_ls = [0] * nfreq
            rms_aod_phi_ls = [0] * nfreq
            rms_aod_theta_ls = [0] * nfreq
            no_path = 1
        else:
            rms_aoa_phi_ls, rms_aoa_theta_ls, rms_aod_phi_ls, rms_aod_theta_ls = [], [], [], []
            for ifreq in range(nfreq):
                npath = np.sum(self.pl_ls[ifreq,:] <= fspl_ls[ifreq]+70)
                if npath > 0:
                    pl_consider = self.pl_ls[ifreq,:npath]
                    pl_min = np.min(pl_consider)
                    w = 10**(-0.1*(pl_consider-pl_min))
                    w = w / np.sum(w)

                    _mean = w.dot(self.ang[:npath,0])
                    rms_aoa_phi_ls.append(np.sqrt(w.dot((self.ang[:npath,0]-_mean)**2)))
                    _mean = w.dot(self.ang[:npath,1])
                    rms_aoa_theta_ls.append(np.sqrt(w.dot((self.ang[:npath,1]-_mean)**2)))
                    _mean = w.dot(self.ang[:npath,2])
                    rms_aod_phi_ls.append(np.sqrt(w.dot((self.ang[:npath,2]-_mean)**2)))
                    _mean = w.dot(self.ang[:npath,3])
                    rms_aod_theta_ls.append(np.sqrt(w.dot((self.ang[:npath,3]-_mean)**2)))
                else:
                    rms_aoa_phi_ls.append(0)
                    rms_aoa_theta_ls.append(0)
                    rms_aod_phi_ls.append(0)
                    rms_aod_theta_ls.append(0)

        return no_path, rms_aoa_phi_ls, rms_aoa_theta_ls, rms_aod_phi_ls, rms_aod_theta_ls

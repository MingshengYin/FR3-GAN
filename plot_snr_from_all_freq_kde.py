...
if if_plot:
    ...
    with open('snr_data_12GHz_train.pkl', 'rb') as file:
        ...
        best_snr_lower_freq_beam_train.append(temp[0])
        best_snr_high_freq_train.append(temp[1])
    ...
    with open('snr_data_12GHz_real.pkl', 'rb') as file:
        temp = pickle.load(file)
        best_snr_lower_freq_beam_real.append(temp[0])
        best_snr_high_freq_real.append(temp[1])
    ...
    for i in range(3):
        sns.kdeplot(best_snr_lower_freq_beam_real[i], best_snr_high_freq_real[i], levels=5, fill = True, ax=ax[0][i])
        sns.set_style("whitegrid")
        ax[0][i].set_xlim(-60,40)
        ax[0][i].set_ylim(-60,40)
        ax[0][i].grid()
        ax[0][i].set_xlabel('SNR - beam selected by 6GHz')
        ax[0][i].set_ylabel('SNR - beam selected by '+label_ls[i])
    for i in range(3):
        sns.kdeplot(best_snr_lower_freq_beam_train[i], best_snr_high_freq_train[i], levels=5, fill = True, ax=ax[1][i])
        sns.set_style("whitegrid")
        ax[1][i].set_xlim(-60,40)
        ax[1][i].set_ylim(-60,40)
        ax[0][i].grid()
        ax[1][i].set_xlabel('SNR - beam selected by 6GHz')
        ax[1][i].set_ylabel('SNR - beam selected by '+label_ls[i])

    fig.text(0.5, 0.97, 'Ray Tracing Samples', ha='center')
    fig.text(0.5, 0.49, 'Generated Links', ha='center')
    fig.tight_layout()
    plt.savefig('0225-6GHz-reduce70%-New/snr-beam-selected.png', dpi = 400)
    plt.show()
